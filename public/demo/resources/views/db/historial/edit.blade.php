                                    ...
                                    <form class="form-group form-prevent-multiple-submit" method="post"
                                        ...
                                        <div class="form-group">
                                            <select class="chosen-select" name="razon" id="razonSelect">
                                                <option disabled>Seleccionar Razón</option>
                                                @foreach($razones as $razonOption)
                                                    <option value="{{$razonOption->razon}}" data-razon-id="{{$razonOption->id}}" @if($razonOption->id == $inspeccion->dbredb_id) selected @endif>{{$razonOption->razon}}</option>
                                                @endforeach
                                                <option value="otro" data-razon-id="NULL" @if($inspeccion->dbredb_id === null) selected @endif>Otro</option>
                                            </select>

                                            <div id="otraRazon" style="display: none;">
                                                <input type="text" name="otraRazon" id="otraRazonInput" placeholder="Ingrese otra razón" @if($inspeccion->dbredb_id === null) value="{{$inspeccion->razon}}" @endif>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" name="motivo" value="{{ $inspeccion->motivo }}">
                                        </div>
                                        <label for="detalle">Detalle de la inspección:</label><br>
                                            <textarea id="detalle" name="detalle" rows="4" cols="200"> {{ $inspeccion->detalle }}</textarea>
                                            <div class="form-group">
...
$(document).ready(function() {
    ...
    $('#razonSelect').change(function() {
        ...
        var otraRazonInput = $('#otraRazonInput');
