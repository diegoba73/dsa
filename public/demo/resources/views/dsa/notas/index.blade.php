        ...
        <div class="container-fluid">
            <div class="header-body">
                ...
                <nav class="navbar navbar-left navbar navbar-dark" id="navbar-main">
                    ...
                    {{ Form::open(['route' => 'dsa_notas_index', 'method' => 'GET', 'class' => 'form-inline pull-right']) }}
                    ...
                    <div class="form-group mr-1">
                            <button type="submit" class="btn btn-secondary btn-fab btn-fab-mini btn-round" href="{{ route('dsa_notas_index') }}">
                ...
                <div class="card bg-gradient-default shadow">
                    <div class="card-header bg-gradient-default border-0">
                        <div class="row align-items-center">
                            ...
                            <div class="col text-right">
                                <a href="{{ route('dsa_notas_create') }}" class="btn btn-sm btn-primary">Nueva Muestra</a>
                            </div>
                        </div>
                    </div>
                    <div class="table-responsive">
                        ...
                        <table class="table align-items-center table-dark">
                            ...
                            <tbody>
                            @foreach ($dsanotas as $dsanota)
                                <tr>
                                    <td class="text-center">{{ $dsanota->numero}}</td>
                                    <td>{{ date('d-m-Y', strtotime($dsanota->fecha)) }}</td>
                                    <td>{{ $dsanota->descripcion}}</td>
                                    <td>{{ $dsanota->user->usuario}}</td>
                                    <td class="td-actions text-left">
                                        <a href="{{ url('/dsa/notas/'.$dsanota->id.'/edit')}}" title="Editar" rel="tooltip" class="btn btn-primary btn-round">
                        ...
                        {{ $dsanotas->appends(request()->except('page'))->links() }}
                        </ul>
                    </div>
                </div>
            </div>
