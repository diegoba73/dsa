...
class MuestraController extends Controller
{
     ...
     */
     ...
     public function index(Request $request)
     {
         ...
         return view('lab.muestras.index', compact(
             'muestras', 'remitentes', 'departamentos', 'filters', 'year'
         ));
     }     
     ...
     private function buildMuestraQuery($user, $filters)
     {
         ...
         // Obtener IDs de departamentos y roles dinámicamente usando los nombres de columna correctos
         $dlDepartmentId = Departamento::where('departamento', 'DL')->value('id'); // Departamento de Laboratorio
         $dsbDepartmentId = Departamento::where('departamento', 'DSB')->value('id'); // Departamento de Salud Bucal
         $dsoDepartmentId = Departamento::where('departamento', 'DSO')->value('id'); // Departamento de Salud Ocular
         $dsaDepartmentId = Departamento::where('departamento', 'DSA')->value('id'); // Departamento de Salud Ambiental (nuevo)
         $adminRoleId = Role::where('name', 'Admin')->value('id'); // Rol de Administrador
         $clienteRoleIds = Role::whereIn('name', ['Cliente', 'Institucion'])->pluck('id')->toArray(); // Roles de Clientes
     
         // Aplicar restricciones según el rol y departamento del usuario
         if (in_array($user->role_id, $clienteRoleIds)) {
             // Clientes: solo ven sus propias muestras
             $muestras->whereHas('remitente', function ($query) use ($user) {
                 $query->where('user_id', $user->id);
             });
         } elseif ($user->departamento_id == $dlDepartmentId || $user->departamento_id == $dsaDepartmentId) {
             // Usuarios del departamento DL o DSA: pueden ver todas las muestras
             // No aplicamos restricciones adicionales
         } elseif ($user->departamento_id == $dsoDepartmentId && $user->role_id == $adminRoleId) {
             // Usuarios del departamento DSO con rol Administrador: ven muestras de DSO y DSB
             $muestras->whereIn('muestras.departamento_id', [$dsoDepartmentId, $dsbDepartmentId]);
         } else {
             // Otros usuarios: ven solo las muestras de su departamento
             $muestras->where('muestras.departamento_id', $user->departamento_id);
         }
    ...
    public function batchAction(Request $request)
    {
        ...
        if ($action === 'batchAction') {
            ...
            // Obtener el departamento para la factura
            $departamento_id = $departamentos->first();
            ...
            $factura->departamento_id = $departamento_id;
            $factura->save();
