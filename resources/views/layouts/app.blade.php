...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="{{ $class ?? '' }}">
        @auth()
            ...
            @include('layouts.navbars.sidebarredb')
            @endif
...
$(document).ready(function() {
    $('#nomenclador-select-pre, #ensayo-select-pre').on('change', function() {
        var selectedNomencladors = $('#nomenclador-select-pre').val();
        var selectedEnsayos = $('#ensayo-select-pre').val();
        var tableBody = $('#selected-nomencladors-table tbody');

        tableBody.empty();

        // Agregar nomencladores seleccionados
        $.each(selectedNomencladors, function(index, value) {
            var row = $('<tr></tr>');
            row.append($('<td></td>').text($('#nomenclador-select-pre option[value="' + value + '"]').text()));
            row.append($('<td></td>').text('Valor: ' + $('#nomenclador-select-pre option[value="' + value + '"]').data('valor')));
            row.append($('<td></td>').append($('<input type="number" class="form-control cantidad" name="nomenclador_cantidades[' + value + ']" placeholder="Cantidad">')));
            row.append($('<td></td>').append($('<input type="text" class="form-control subtotal" readonly>')));

            tableBody.append(row);
        });

        // Agregar ensayos seleccionados
        $.each(selectedEnsayos, function(index, value) {
            var row = $('<tr></tr>');
            row.append($('<td></td>').text($('#ensayo-select-pre option[value="' + value + '"]').text()));
            row.append($('<td></td>').text('Costo: ' + $('#ensayo-select-pre option[value="' + value + '"]').data('costo')));
            row.append($('<td></td>').append($('<input type="number" class="form-control cantidad" name="ensayo_cantidades[' + value + ']" placeholder="Cantidad">')));
            row.append($('<td></td>').append($('<input type="text" class="form-control subtotal" readonly>')));
    ...
    // Recalculate subtotals and total when quantity changes
    ...
    function recalculateSubtotals() {
        ...
        const subtotalClass = '.subtotal';
        const totalId = '#total-factura';

        var subtotals = $(cantidadClass).map(function() {
            var cantidad = parseFloat($(this).val());
            var costo = parseFloat($(this).closest('tr').find('td:eq(1)').text().split(' ')[1]);
            var subtotal = cantidad * costo;
            $(this).closest('tr').find(subtotalClass).val(subtotal.toFixed(2));
            return subtotal;
        }).get();
