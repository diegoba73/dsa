                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="alert alert-default">
                                    <strong class="card-title">Pedido de Modificación de Registro R.E.D.B. {{$redb->establecimiento}}</strong>
                                </div>
                                <div class="card-body">
                                    <form id="formulario" method="post" action="{{ route('db_redb_update_modificacion', $redb->id) }}" enctype="multipart/form-data" onsubmit="return validarFormulario()">
                                        ...
                                        @if(in_array(Auth::user()->role_id, [9, 16, 17]))
                                        <div class="row">
                                            <div class="col">
                                                <div class="form-group">
                                        ...
                                        @if($ultimoHistorial)
                                            ...
                                            <div class="row">
                                                <div class="col">
                                                    @php
                                                        $ultimoTramite = $redb->dbtramites->last();
                                                    @endphp
                                                    @if (Auth::id() != $historial->user_id)
                                                        @if ($ultimoTramite && $ultimoTramite->estado != 'MODIFICADO')
                                                        ...
                                                        @endif
                                                    @elseif ((Auth::user()->role_id == 15) && ($redb->dbtramites->contains('estado', 'INICIADO')))
                                        ...
                                        <a class="btn btn-default btn-close" href="{{ URL::previous() }}">Cancelar</a>
                                            @if (((Auth::user()->role_id <> 15) && ($redb->dbtramites->contains('estado', 'INICIADO'))) || ((Auth::user()->role_id == 15) && ($redb->dbtramites->contains('estado', 'REVISADO'))))
                                            ...
                                            @endif
                                            @if ((Auth::user()->role_id == 9) && $redb->dbtramites->contains('estado', 'EVALUADO'))
                                            ...
                                            @endif
                                            @if ((Auth::user()->role_id == 9) && $redb->dbtramites->contains('estado', 'EVALUADO'))
                                                    <button type="submit" class="btn btn-success pull-center button-prevent-multiple-submit" name="submitType" value="aprobar">
                                                        <i class="spinner fa fa-spinner fa-spin" style="display:none;"></i>
                                                        APROBAR
                                                    </button>
                                            @endif
                                            @if((Auth::user()->id == 4) &&
                                                $redb->dbtramites->contains('estado', 'APROBADO'))
                                                <button type="submit" class="btn btn-success pull-center button-prevent-multiple-submit" name="submitType" value="modificar">
                                                    <i class="spinner fa fa-spinner fa-spin" style="display:none;"></i>
                                                    Modificar
                                                </button>
                                            @endif
