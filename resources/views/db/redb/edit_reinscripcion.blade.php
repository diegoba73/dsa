                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="alert alert-default">
                                <strong class="card-title">Reinscripcion de Registro R.E.D.B. Nro: {{$redb->numero}} - {{$redb->establecimiento}}</strong>
                            </div>
                            <div class="card-body">
                                <form id="formulario" method="post" action="{{ route('db_redb_update_reinscripcion', $redb->id) }}" enctype="multipart/form-data" onsubmit="return validarFormulario()">
                                    {{ csrf_field() }}
                                    @method('PUT')
                                    <div class="row">
                                        <div class="col-4">
                                    ...
                                    <div class="row">
                                        ...
                                        @if (Str::contains($tramite->tipo_tramite, 'BAJA'))
                                            ...
                                            @endif
                                        @endif
                                        @if ($tramite && $tramite->estado === 'REINSCRIPTO')
                                            <div class="alert alert-success">
                                                <strong>EL TRAMITE DE REINSCRIPCION SE APROBÓ.</strong>
                                            </div>
                                        @endif
                                        </div>
                                        <div class="col-md-12 text-center">
                                            ...
                                            @if (in_array(Auth::user()->role_id, [1, 9]) && Str::contains($tramite->tipo_tramite, 'BAJA ESTABLECIMIENTO') && ($redb->dbbaja_id === null || $redb->dbbaja_id == 0))
                                                <a href="{{ route('db_baja_create', ['redb_id' => $tramite->dbredb_id]) }}" class="btn btn-danger pull-center button-prevent-multiple-submit" title="Baja" rel="tooltip">
                                                    <i class="spinner fa fa-spinner fa-spin" style="display:none;"></i>
                                                    DAR DE BAJA
                                                </a>
                                            @else
                                                <!-- Enviar Modificación (solo para rol PRODUCTOR con estado distinto a INICIADO) -->
                                                @if (Auth::user()->role_id == 15 && !in_array($tramite->estado, ['INICIADO', 'REENVIADO POR EMPRESA', 'APROBADO', 'REINSCRIPTO']))
                                                ...
                                                @if (
                                                    ...
                                                    && Str::contains(strtoupper($tramite->tipo_tramite), 'REINSCRIBIR') === true
                                                    && (!($redb->localidad->zona !== 'nc' && $tramite->estado === 'INICIADO') || $tramite->estado === 'ENVIADO A NC' || $tramite->estado === 'REENVIADO POR EMPRESA')
                                                )
                                                    <button type="submit" id="reinscribir" class="btn btn-success pull-center button-prevent-multiple-submit" name="submitType" value="reinscribir">
                                                        <i class="spinner fa fa-spinner fa-spin" style="display:none;"></i>
                                                        ACEPTAR REINSCRIPCION
                                                    </button>
                                                @endif
                                            @endif
                                        </div>
...
<script>
    $(document).ready(function() {
        ...
        function validarFormulario(event) {
            ...
            // Verificar si se está enviando el formulario mediante el botón de REINSCRIBIR
            if (event.submitter && event.submitter.id === 'reinscribir') {
                if (expediente.value.trim() === '') {
                    mensajesAlerta.push('Por favor, completa el campo de NÚMERO DE EXPEDIENTE para reinscribir.');
                }
            }

            // Verificar si se está enviando el formulario mediante los botones de APROBAR o ACEPTAR ÁREA
