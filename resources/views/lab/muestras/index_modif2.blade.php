                        ...
                        <div class="card-body">
                            <nav class="navbar navbar-left navbar navbar-dark" id="navbar-main">
                                ...
                                {{ Form::open(['route' => 'lab_muestras_index', 'method' => 'GET', 'class' => 'form-inline pull-right']) }}
                                    ...
                                    @if ((Auth::user()->departamento_id == 1) || (Auth::user()->departamento_id == 5) || (Auth::user()->id == 30 || Auth::user()->id == 2885))
                                    <div class="form-group mr-2 mb-2">
                                        <div class="input-group input-group-alternative">
                        ...
                        <table class="table table-striped align-items-center table-dark">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    @if (in_array(Auth::user()->departamento_id, [2, 3, 4]) || Auth::user()->id == 4)
                                        <th scope="col">Remite</th>
                                    @endif
                                    @if (in_array(Auth::user()->departamento_id, [1, 4]) || Auth::user()->id == 4)
                                        <th scope="col" class="text-center">Dpto</th>
                                    @endif
                                    @if (Auth::user()->departamento_id == 4 || Auth::user()->id == 4)
                                ...
                                @foreach ($muestras as $muestra)
                                    <tr>
                                        <form action="{{ route('lote') }}" method="POST">
                                            @csrf
                                            <td style="width:1px">
                                                <input type="checkbox" class="control-input" id="selected" name="selected[]" value="{{ $muestra->id }}">
                                            </td>
                                            <td style="width:20px">
                                                <a class="text-white" href="{{ url('/lab/muestras/'.$muestra->id.'/show') }}">{{ $muestra->numero }}</a>
                                            </td>
                                            <td style="width: 130px">
                                                <a class="text-white" href="{{ url('/lab/muestras/'.$muestra->id.'/show') }}">
                                                    <strong>{{ $muestra->matriz->matriz }} /</strong>
                                                    {{ Str::limit($muestra->muestra, 20) }}
                                                </a>
                                            </td>
                                            @if (in_array(Auth::user()->departamento_id, [2, 3, 4]) || Auth::user()->id == 4)
                                                <td style="width:20%">
                                                    <a class="text-white" href="{{ url('/lab/muestras/'.$muestra->id.'/show') }}">
                                                        {{ Str::limit($muestra->remitente->nombre, 20) }}
                                                    </a>
                                                </td>
                                            @endif
                                            @if (in_array(Auth::user()->departamento_id, [1, 4]) || Auth::user()->id == 4)
                                                <td style="width:20px" class="text-center">
                                                    @php
                                                        $badgeColor = is_null($muestra->aceptada) ? 'warning' : ($muestra->aceptada ? 'success' : 'danger');
                                                        $deptoBadges = [
                                                            1 => 'DL',
                                                            2 => 'DB',
                                                            3 => 'DSB',
                                                    ...
                                                    <span class="badge badge-pill badge-{{ $badgeColor }}">
                                                        {{ $deptoBadges[$muestra->departamento_id] ?? '' }}
                                                    </span>
                                                </td>
                                            @endif
                                            @if (Auth::user()->departamento_id == 4 || Auth::user()->id == 4)
                                                <td style="width:20px" class="text-center">
                                                    ...
                                                    @endphp
                                                    @foreach ($areas as $field => $badge)
                                                        @if (!is_null($muestra->$field))
                                                            <span class="badge badge-pill badge-info">{{ $badge }}</span>
                                                        @endif
                                                    @endforeach
                                                </td>
                                            @endif
                                            ...
                                            <td style="width:20px">
                                                @if (strtotime($muestra->fecha_salida) > 0)
                                                    {{ date('d-m-Y', strtotime($muestra->fecha_salida)) }}
                                                @else
                                                    <span class="badge badge-pill badge-{{ is_null($muestra->aceptada) ? 'warning' : ($muestra->aceptada ? 'success' : 'danger') }}">
                                                        SIN SALIDA
                                                    </span>
                                                @endif
                                            </td>
                                            <td style="width:10px" align="center">
                                                @if ($muestra->revisada)
                                                    <span class="fas fa-check fa-2x text-green"></span>
                                                @endif
                                            </td>
                                            <td style="width:10px" align="center">
                                                    ...
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                                            @if (($muestra->revisada === null) || ($muestra->revisada === 0))
                                                                <a class="dropdown-item" href="{{ route('devolver_muestra', $muestra->id) }}">
                                                                    <i class="far fa-arrow-alt-circle-left"></i> Devolver
                                                                </a>
                                                                @if ($muestra->aceptada === null)
                                                                    <a class="dropdown-item" href="{{ route('aceptar_muestra', $muestra->id) }}">
                                                                        <i class="fas fa-check-circle"></i> Aceptar
                                                                    </a>
                                                                    <a class="dropdown-item" href="{{ route('lab_muestras_rechazo', $muestra->id) }}">
                                                                ...
                                                                @elseif ($muestra->aceptada === 0)
                                                                    ...
                                                                    <a class="dropdown-item" href="{{ route('lab_muestras_rechazo', $muestra->id) }}">
                                                                        <i class="fas fa-times-circle"></i> Formulario Rechazo
                                                                    </a>
                                                                    <a class="dropdown-item" href="{{ url('/lab/muestras/'.$muestra->id.'/imprimir_rechazo') }}" target="_blank">
                                                                        <i class="fas fa-print"></i> Imprimir Rechazo
                                                                    </a>
                                                                @elseif ($muestra->aceptada === 1)
                                                                    <a class="dropdown-item" href="{{ route('lab_muestras_rechazo', $muestra->id) }}">
                                                                        <i class="fas fa-times-circle"></i> Rechazar
                                                                    </a>
                                                                    <a class="dropdown-item" href="{{ url('/lab/muestras/'.$muestra->id.'/ht') }}" target="_blank">
                                                                        <i class="fas fa-file-alt"></i> Hoja de Trabajo
                                                                    </a>
                                                                    <a class="dropdown-item" href="{{ route('lab_muestras_resultado', $muestra->id) }}">
                                                                        <i class="fas fa-file-contract"></i> Resultado
                                                                    </a>
                                                                @endif
                                                                @if ($muestra->cargada === 1)
                                                                    <a class="dropdown-item" href="{{ url('/lab/muestras/'.$muestra->id.'/imprimir_resultado') }}" target="_blank">
                                                            ...
                                                            @if ($muestra->revisada === 1)
                                                                ...
                                                                <a class="dropdown-item" href="{{ url('/lab/muestras/'.$muestra->id.'/imprimir_resultado_firma') }}" target="_blank">
                                                                    <i class="fas fa-file-signature"></i> Imprimir Resultado Firmado
                                                                </a>
                                                                @if ($muestra->traducida === 1)
                                                                    <a class="dropdown-item" href="{{ url('/lab/muestras/'.$muestra->id.'/imprimir_traducido') }}" target="_blank">
                                                                        <i class="fas fa-file-signature"></i> Imprimir Resultado Traducido
                                                                    </a>
                                                                    <a class="dropdown-item" href="{{ url('/lab/muestras/'.$muestra->id.'/imprimir_traducido_firma') }}" target="_blank">
                                                                        <i class="fas fa-file-signature"></i> Imp. Res. Traducido Firmado
                                                                    </a>
                                                                @endif
                                                            @endif
                                                        </div>
                                                    </div>
                                                @elseif (in_array(Auth::user()->departamento_id, [2, 3, 4, 6]) && $muestra->revisada === 1)
                                                    <div class="dropdown">
                                                        <a class="btn btn-sm btn-icon-only text-light" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                            <i class="fas fa-ellipsis-v"></i>
                                                        </a>
