...
class MuestraController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ((Auth::user()->role_id == 1 || Auth::user()->role_id == 2 || Auth::user()->role_id == 8)&&(Auth::user()->departamento_id == 1)){
                ...
                $muestras = Muestra::with('remitente')
                    ...
                    $muestras = Muestra::with('remitente')
                    ->where('muestras.departamento_id', '=', 2)
                    ->orwhere('muestras.departamento_id', '=', 3)
                    ->orwhere('muestras.departamento_id', '=', 4)
                    ->orderBy('muestras.id', 'DESC')
                    ->id($id)
                    ->numero($numero)
            ...
            } elseif (Auth::user()->departamento_id == 16){

                $muestras = Muestra::with('remitente')
                ->where('muestras.departamento_id', '=', 8)
                ->orderBy('muestras.id', 'DESC')
                ->id($id)
                ->numero($numero)
                ->tipo($tipo_muestra)
            ...
            } elseif (Auth::user()->departamento_id == 17){

                $muestras = Muestra::with('remitente')
                ->where('muestras.departamento_id', '=', 9)
                ->orderBy('muestras.id', 'DESC')
                ->id($id)
                ->numero($numero)
                ->tipo($tipo_muestra)
    ...
    public function update(Request $request, $id)
    {
        ...
        $muestra = Muestra::findOrFail($id);
            ...
            // Obtener los valores originales antes de la actualización
        $remitenteOriginalId = $muestra->remitente_id;
        $solicitanteOriginal = $muestra->solicitante;
